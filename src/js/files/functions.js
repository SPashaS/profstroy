...
// 	}
// }
// export let bodyLock = (delay = 0) => { //500
// 	let body = document.querySelector("body");
// 	if (bodyLockStatus) {
// 		let lock_padding = document.querySelectorAll("[data-lp]");
// 		for (let index = 0; index < lock_padding.length; index++) {
...
export let bodyUnlock = (delay = 0) => {
  if (!bodyLockStatus) return; // Если уже в процессе, выходим
...
export function tabs() {
  ...
  function initTabs(tabsBlock) {
    ...
    if (tabsContent.length) {
      //////
      tabsImages = Array.from(tabsImages).filter(
...
export function menuInit() {
  ...
  let menuLinks = document.querySelectorAll('.menu__list .menu-item a'); // Все ссылки в меню
  if (iconMenu) {
...
// export function menuInit() {
// 	let iconMenu = document.querySelector(".icon-menu");
// 	let menuLinks = document.querySelectorAll(".menu a"); // Все ссылки в меню
// 	let headerService = document.querySelector('.header-services');
// 	let spollerLinks = document.querySelectorAll(".menu [data-spoller]"); // Элементы-спойлеры

// 	// Функция для закрытия всех открытых меню
// 	function closeAllMenus() {
// 		document.documentElement.classList.remove("menu-open");
// 		if (headerService) {
// 			headerService.classList.remove("header-services_open");
// 		}
// 		bodyUnlock();
// 	}

// 	// Функция для обработки клика по значку меню
// 	function handleMenuToggle() {
// 		// Определяем текущее разрешение
// 		const screenWidth = window.innerWidth;

// 		// Для разрешений меньше 768px или между 768px и 1280px
// 		// if (screenWidth < 768 || (screenWidth >= 768 && screenWidth < 1280)) {
// 		if (screenWidth < 768 || screenWidth < 1280) {
// 			if (!document.documentElement.classList.contains("menu-open")) {
// 				closeAllMenus();
// 				document.documentElement.classList.add("menu-open");
// 				if (screenWidth < 768 && bodyLockStatus) {
// 					bodyLock(); // Блокировка скролла только для маленьких экранов
// 				}
// 			} else {
// 				closeAllMenus();
// 				if (screenWidth < 768 && !bodyLockStatus) {
// 					bodyUnlock(); // Блокировка скролла только для маленьких экранов
// 				}
// 			}
// 		}
// 	}

// // Обработчик кликов на ссылки меню
// function handleLinkClick(e) {
// 	// Если это спойлер, ничего не делаем
// 	if (e.target.hasAttribute("data-spoller")) return;

// 	// Закрываем меню для всех других ссылок
// 	closeAllMenus();
// }

// 	// Обработчик для headerService
// 	function handleHeaderServiceToggle() {
// 		if (!headerService.classList.contains('header-services_open')) {
// 			closeAllMenus();
// 			headerService.classList.add('header-services_open');
// 		} else {
// 			headerService.classList.remove('header-services_open');
// 			bodyUnlock();
// 		}
...
// 	// Добавляем слушатель клика для иконки меню
// 	if (iconMenu) {
// 		iconMenu.addEventListener("click", handleMenuToggle);
// 	}

// 	// Добавляем обработчики кликов на ссылки меню
// 	if (menuLinks.length) {
// 		menuLinks.forEach(link => {
...
// 	// Добавляем обработчик кликов на headerService
// 	if (headerService) {
// 		headerService.addEventListener("click", handleHeaderServiceToggle);
// 	}
...
// 	const headerService = document.querySelector(".header-services");
...
// 	// Обработка клика по значку меню
// 	function handleMenuToggle() {
// 			const screenWidth = window.innerWidth;
...
// 					if (!document.documentElement.classList.contains("menu-open")) {
// 							document.documentElement.classList.add("menu-open");
// 							if (screenWidth < 768) bodyLock(); // Блокировка скролла
// 					} else {
// 							closeAllMenus();
// 					}
// 			}
...
// 	// Обработка кликов на ссылки меню
// 	function handleLinkClick(e) {
// 			if (e.target.hasAttribute("data-spoller")) return;
// 			closeAllMenus();
// 	}

// 	// Обработка клика на headerService
// 	function handleHeaderServiceToggle() {
// 			if (!headerService.classList.contains("header-services_open")) {
// 					closeAllMenus();
// 					headerService.classList.add("header-services_open");
...
// 	if (iconMenu) iconMenu.addEventListener("click", handleMenuToggle);
// 	menuLinks.forEach((link) => link.addEventListener("click", handleLinkClick));
// 	if (headerService) headerService.addEventListener("click", handleHeaderServiceToggle);
