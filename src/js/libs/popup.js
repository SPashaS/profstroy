...
//       bodyLockDelay: 0, // Задержка блокировки скролла  500
    ...
    if (this.targetOpen.element) {
      ...
      if (!this._reopen) {
        if (
          ...
          bodyLock(); // Блокируем прокрутку только если не заблокировано меню или другой попап
        }
      } else this._reopen = false;
  ...
  close(selectorValue) {
    ...
    if (!this._reopen) {
      ...
      if (
        ...
        bodyUnlock(); // Разблокируем только если НЕТ открытых попапов и меню
      }
